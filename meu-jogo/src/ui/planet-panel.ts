...
function planetName(planeta: Planeta): string {
  // Tries dados.nome first (added by the procedural-names PR) and falls
  // back to the type label so this panel works independent of that PR.
  const dados = planeta.dados as { nome?: string; tipoPlaneta: string };
  return dados.nome || nomeTipoPlaneta(dados.tipoPlaneta);
}
